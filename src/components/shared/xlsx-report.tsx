"use client";
// EMET · Componente compartido de reporte semanal de asistencia en Excel (.xlsx)
// Formato unificado para admin y RH: colores por empleado, información de
// vacaciones/permisos/incapacidades, mismo diseño que el checador legado.
...
/** Colores semánticos para estados de ausencia */
...
export function XlsxReportButton({
        ...
        for (const emp of empBlocks) {
          ...
          for (const d of emp.days) {
            ...
            row.eachCell((cell, colNumber) => {
              ...
              // Colorear la columna de comentarios según el estado
              if (colNumber === 9 && d.statusLabel) {
                const statusColor = STATUS_COLORS[d.statusLabel] || { bg: "F1F5F9", fg: "475569" };
                cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF" + statusColor.bg } };
                cell.font = { bold: true, size: 10, color: { argb: "FF" + statusColor.fg } };
              }
            });
  ...
  return (
    ...
    >
      <IconDownload className="w-3.5 h-3.5" /> {busy ? "Generando…" : label}
    </button>
  );
}
