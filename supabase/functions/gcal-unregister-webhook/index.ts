// EMET · Edge Function: gcal-unregister-webhook
// Cancela un webhook registrado con Google Calendar
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!webhook) {
      return Response.json({ ok: true, message: "No había webhook registrado" }, { headers: cors });
    }

    // Cancelar el webhook con Google
    ...
    // Eliminar el webhook de la base de datos (incluso si Google falla)
    await admin.from("google_calendar_webhooks").delete().eq("id", webhook.id);

    if (!stopRes.ok && stopRes.status !== 404) {
      const errJson = await stopRes.json();
      return Response.json({ ok: false, error: "google-rechazo-la-cancelacion", detail: errJson }, { status: 502, headers: cors });
    }

    return Response.json({
      ok: true,
      message: "Webhook cancelado correctamente",
    }, { headers: cors });
  } catch (e) {
    console.error("gcal-unregister-webhook error:", e);
